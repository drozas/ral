package body http10 is
   ...
   function HostValido(Peticion: in Petition_Type) return Boolean is
   --funcion auxiliar que determina si es válido el host de 1.0
      EsValido:Boolean:=FALSE;
   begin
        if ASU.To_String(Peticion.Uri.Host)= "www.localdomain.org"  then
           ...
           if ExisteCampo(Peticion,ASU.To_Unbounded_String("Host")) then
                ...
                if CoincideCampo(peticion,ASU.To_Unbounded_String("Host"),
                                 ASU.To_Unbounded_String("www.localdomain.org")) then
                EsValido:=TRUE;
                else
...
-------------------------------------------------------------------------------------------------------
   ...
   procedure Attend_Petition(Peticion: in Petition_Type; Conexion: in out TCP.Connection) is
   ...
   begin
      ...
      if Peticion.Method= Petition_Analysis.GET then
         Attend_GET_Petition(Peticion,Conexion);
      elsif Peticion.Method= Petition_Analysis.HEAD then
         Attend_HEAD_Petition(Peticion,Conexion);
      else
...
------------------------------------------------------------------------------------------------------
   procedure Attend_GET_Petition(Peticion: in Petition_Type; Conexion: in out TCP.Connection) is
...
procedure Attend_HEAD_Petition(Peticion: in Petition_Type; Conexion: in out TCP.Connection) is
      fichero: Ada.Streams.Stream_IO.File_Type;
      TamFichero: Natural;
      Ruta: ASU.Unbounded_String;
   begin
