...
-----------------------------------------------------------------------------------
   procedure esDominioServido(Host:in out  Asu.Unbounded_String;
     ...
     begin
        ...
        while (I<=Configuracion.NTotalDominios) and ( not esServido) loop
           Ada.Text_Io.Put_Line("Comparando : " & ASU.TO_String(Host) & " con : " & ASU.To_String(Configuracion.Dominios(I)));
           ...
           Ada.Text_Io.Put_Line("Comparando : " & ASU.TO_String(Host) & " con : " & ASU.To_String(Domain_Con_Port));
           if Configuracion.Dominios(I)= Host then
              --Si lo encontramos sin port, simplemente cambiamos el booleano
              Ada.Text_Io.Put_Line("¡El dominio es local, y sin port! -> no modificamos host");
              esServido:= True;
              ...
              Ada.Text_Io.Put_Line("¡Es dominio local, y lleva port!");
              EsServido:= True;
              Host:= Configuracion.Dominios(I);
              Ada.Text_Io.Put_line("Nuevo valor de host, eliminando el port: " & ASU.To_String(Host));
           end if;
           I:=i+1;
        end loop;
   ...
   begin
      ...
      Ada.Text_Io.Put_Line("valor del uri con port : " & Asu.To_String(ValorURIHost_Con_Port));
      ...
      if Peticion.Version=Petition_Reply_Analysis.V10 then
         ...
         if ValorURIHost/=ASU.Null_Unbounded_String then
            ...
            if EsServido  then
               ...
               Ada.Text_Io.Put_Line("hay uri, y lo tenemos -> hostDevuelto: " & ASU.To_String(HostDevuelto));
              ...
              or ValorURIHost_Con_Port=Ip_Con_Port then
               ...
               Ada.Text_Io.Put_Line("hay uri, y es ip o ip+port -> devolvemos el de por defecto");
            else
               HostDevuelto:= ASU.Null_Unbounded_String;
               Ada.Text_Io.Put_Line("hay uri, pero no la tenemos-> hostDevuelto: " & ASU.To_String(HostDevuelto));
            end if;

         elsif ExisteCampo(Peticion,ASU.To_Unbounded_String("Host")) then
            --Si noy hay uri, pero hay campo host; es obligatorio que sea uno de los servido
            ...
            if EsServido then
               ...
               Ada.Text_Io.Put_line("no hay uri, pero hay host y lo servimos-> hostDevuelto: " & ASU.To_String(HostDevuelto));
              ...
              or ValorCampoHost=ASU.Null_Unbounded_String then
               HostDevuelto:= Get_DominioPorDefecto(Configuracion);
               Ada.Text_Io.Put_Line("no hay uri, pero hay host y es ip o ip+port o vacio-> hostDevuelto: " & ASU.To_String(HostDevuelto));

            else
               HostDevuelto:=ASU.Null_Unbounded_String;
               Ada.Text_Io.Put_Line("no hay uri, hay host, pero ni lo servimos ni es nuestra ip ->hostDevuelto: " & ASU.To_String(HostDevuelto));

            end if;
         else
            HostDevuelto:= Get_DominioPorDefecto(Configuracion);
            Ada.Text_Io.Put_Line("no hay uri, ni host; le asignamos el de por defecto-> hostDevuelto: " & ASU.To_String(HostDevuelto));
         end if;
      ...
      --SI ES VERSION 1.1...
         ...
         if ExisteCampo(Peticion,ASU.To_Unbounded_String("Host")) then
            ...
            if ValorURIHost/=ASU.Null_Unbounded_String then
               ...
               if EsServido then
                  HostDevuelto:=ValorURIHost;
                  Ada.Text_Io.Put_Line("hay host. hay uri, y lo servimos. ->hostDevuelto: " & ASU.To_String(HostDevuelto));
               elsif ValorURIHost=ASU.To_String(Get_IpMaquina(Configuracion))
                 or ValorURIHost_Con_Port=Ip_Con_Port then
                  HostDevuelto:= Get_DominioPorDefecto(Configuracion);
                  Ada.Text_Io.Put_Line("hay uri, y es ip o ip+port -> devolvemos el de por defecto");
                  ...
                  HostDevuelto:= ASU.Null_Unbounded_String;
                  Ada.Text_Io.Put_Line("hay host. hay uri, pero no lo servimos. ->hostDevuelto: " & ASU.To_String(HostDevuelto));

               end if;
            else
               --Si la uri esta vacia, miramos el host( es 1.1, tiene que esta
               EsDominioServido(ValorCampoHost,Configuracion,EsServido);
               if EsServido then
                  HostDevuelto:= ValorCampoHost;
                  Ada.Text_Io.Put_Line("hay host, y no hay uri. el host lo servimos. ->hostDevuelto: " & ASU.To_String(HostDevuelto));
               elsif ValorCampoHost=Get_IpMaquina(Configuracion)
                 or ValorCampoHost=Ip_Con_Port then
                 HostDevuelto:= Get_DominioPorDefecto(Configuracion);
                 Ada.Text_Io.Put_Line("no hay uri, pero hay host y es ip o ip+port-> hostDevuelto: " & ASU.To_String(HostDevuelto));
               else
                  HostDevuelto:= ASU.Null_Unbounded_String;
                  Ada.Text_Io.Put_Line("hay host, y no hay uri. el host no lo servimos. ->hostDevuelto: " & ASU.To_String(HostDevuelto));
               end if;
            end if;

         else
            --Si no...fuera, directamente
            HostDevuelto:= ASU.Null_Unbounded_String;
            Ada.Text_Io.Put_Line("no hay host. asi que ni miramos el uri. ->hostDevuelto: " & ASU.To_String(HostDevuelto));
...
begin
      ...
      --LEEMOS EL RESTO DE LA PETICION DEL CGI EN CUALQUIER CASO!
